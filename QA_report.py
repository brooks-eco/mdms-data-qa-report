...
def load_data(filepath, testing_group, filter_date, start, end):
    ...
    for sheet_name in xls.sheet_names:
        ...
        date_col = (
            "SampleDate" if "SampleDate" in df.columns else "date" if "date" in df.columns else None
        )
    ...
    if testing_group is not None:
        ...
        if group_not_found:
            print(
                f"No data for specified test group(s) {group_not_found} found in workbook: required {testing_group} found {groups}"
            )
            return None
...
def join_tables_generic(dfs, joins_required):
    ...
    for left_df_name, join_info in joins_required.items():
        ...
        print(f"Using the join enhanced version of table '{left_df_name}' for summaries and plots.")
    ...
    for table_name in dfs.keys():
        if table_name not in joined_dfs:
            ...
            print(f"Using the workbook version of table '{table_name}' for summaries and plots.")
...
def filter_df(filter, df, task_type=None, task_name=None):
    ...
    for filter_col, filter_condition in filter.items():
        if filter_col in df.columns:
            if isinstance(filter_condition, str) and filter_condition.lower() == "is not null":
                df = df[df[filter_col].notna()]
            elif isinstance(filter_condition, str) and filter_condition.lower() == "is null":
...
def create_single_pie_plot(ax, df, label_col, value_col, title_str, color_map=None):
    ...
    plot_data[label_col] = (
        plot_data[label_col].fillna("missing/not provided").astype(str).str.replace(" ", "\n")
    )
...
def get_global_color_map(df, column_name):
    ...
    return dict(zip(unique_vals, colors, strict=True))
...
def create_plots(joined_dfs, data_summaries, PLOTS_DEFINITION, output_path) -> list[str]:
    ...
    for plot_series_name, config in PLOTS_DEFINITION.items():
        ...
        if table_name in joined_dfs:
            ...
            print(
                f"Error: Table '{table_name}' not found in joined data or summaries for plot '{plot_series_name}'."
            )
            continue
        ...
        for group_key, group_df in groups:
            ...
            all_numeric_groups = (
                all(is_numeric_dtype(df[col]) for col in group_by_cols) if group_by_cols else False
            )

            if all_numeric_groups and group_by_cols:
                title_parts = [
                    f"{col}: {val}" for col, val in zip(group_by_cols, group_key_tuple, strict=True)
                ]
...
def generate_effort_summaries(joined_dfs, summaries_config):
    ...
    for summary_name, config in summaries_config.items():
        ...
        if table_name not in joined_dfs:
            print(f"Error: Table '{table_name}' not found for summary '{summary_name}'.")
            continue

        df = joined_dfs[table_name]
        # filter the dataframe based on the filter specified in the config if it exists
        if "filter" in config:
            df = filter_df(config["filter"], df, task_type="summary", task_name=summary_name)
        ...
        if num_groups > 50:
            ...
            if (
                ...
                group_by_cols = [col for col in group_by_cols if col != "SamplingUnitID"] + [
                    "SamplePointName",
                    "TransectID",
                ]
        ...
        if isinstance(summary_df.columns, pd.MultiIndex):
            ...
            summary_df.columns = [
                (
                    ...
                    if col in summary_funcs and isinstance(func, str) and func != "first"
                    else col
                )
                for col, func in zip(
                    summary_df.columns,
        ...
        for col in summary_df.columns:
            if str(col).endswith("\nunique"):
                summary_df[col] = summary_df[col].apply(
                    lambda x: (
                        "; ".join(sorted([str(v) for v in x if pd.notna(v) and str(v) != ""]))
        ...
        for col, func in col_map.items():
            ...
            if func in ["sum", "max", "min"] and "cover\n" in col.lower():
                ...
                summary_df.loc[
                    (summary_df[col] >= 0) & (summary_df[col] <= 101.0), qa_check_col
                ] = "\u2713"
            ...
            if func == "max" and "Date" in col and min_col_name in summary_df.columns:
                if pd.api.types.is_datetime64_any_dtype(summary_df[col]):
                    ...
                    summary_df.loc[mask, qa_check_col] = (
                        "long survey (" + duration[mask].dt.days.astype(str) + " days)"
                    )
                    # explicit check mark for valid range
            ...
            if func == "count" and nunique_col_name in summary_df.columns:
                if pd.api.types.is_numeric_dtype(summary_df[col]):
                    ...
                    summary_df[qa_check_col] = ""
                    summary_df.loc[
                        summary_df[col] != summary_df[nunique_col_name], qa_check_col
                    ] = "mismatched"
                    # explicit check mark for valid range
                    summary_df.loc[
                        summary_df[col].notna() & (summary_df[col] == summary_df[nunique_col_name]),
                        qa_check_col,
                    ] = "\u2713"
            ...
            if func == "count" and col in count_cols:
                qa_check_col = f"QA Check\n{col} = {first_count_col}"
                summary_df[qa_check_col] = ""
                summary_df.loc[summary_df[col] != summary_df[first_count_col], qa_check_col] = (
                    "mismatched"
                )
                summary_df.loc[
                    summary_df[col].notna() & (summary_df[col] == summary_df[first_count_col]),
                    qa_check_col,
                ] = "\u2713"
            ...
            if (
                func == "count"
                and "SoilMoisture" in col
                and plot_id_count_col in summary_df.columns
            ):
                ...
                # explicit check mark for valid range
                summary_df.loc[
                    summary_df[col].notna() & (summary_df[col] == summary_df[plot_id_count_col]),
                    qa_check_col,
                ] = "\u2713"
...
def qa_outliers(func, col, summary_df):
    ...
    if pd.notna(iqr):
        ...
        summary_df[qa_check_col] = ""
        summary_df.loc[summary_df[col] > upper_bound, qa_check_col] = f"high {col}"
        summary_df.loc[summary_df[col] < lower_bound, qa_check_col] = f"low {col}"
        if func == "count":
            summary_df.loc[summary_df[col].isna() | (summary_df[col] == 0), qa_check_col] = (
                "no data"
            )
            ...
            summary_df.loc[
                ...
                & (summary_df[col] > 0)
                & (summary_df[col] >= lower_bound)
                & (summary_df[col] <= upper_bound),
                qa_check_col,
            ] = "\u2713"
        else:
...
def main():
    ...
    try:
        ...
        data_date = datetime.datetime.strptime(timestamp_str, "%Y%m%d%H%M%S").strftime(
            "%Y-%m-%d %H:%M:%S"
        )
    ...
    for group_name in workbook_data:
        ...
        if not joined_dfs:
            print(
                "No data tables joined. Please check sheet names defined in config match the workbook."
            )
